...
.then(function(data){
	...
	let goldPr = (data[key].priceGold).toFixed(2);
	let retailPr = (data[key].priceRetail).toFixed(2);
	let goldPrAlt = (data[key].priceGoldAlt).toFixed(2);
	let retailPrAlt = (data[key].priceRetailAlt).toFixed(2);				
		out+='<span class="goldPrice">'+goldPrAlt+'</span>';		
		out+='<i class="fas fa-ruble-sign" style="font-size:20px"></i>';
		out+='</div>';						
		out+='<p class="product_price_default">';						
		out+='<span class="retailPrice">'+retailPrAlt+'</span>';
		out+='<i class="fas fa-ruble-sign" style="font-size:20px;color:#a7a7a7;"></i>';	
		out+='</p>';						
		out+='<div class="product_price_points">';						
		out+='<p>Можно купить за 231,75 балла</p>';							
		out+='</div>';	
		out+='<div class="product_units">';						
		out+='<div class="zamkv product_units--select ">';							
		out+='<p class="mkv product_unit--active">За м. кв.</p>';								
		out+='</div>';							
		...
		out+='</p>';							
		out+='</div>';	
		out+='<div class="product__count">';						
		out+='<input class="product__count_input" type="text" value="'+1*1+'"/>';							
		out+='<button class="btn_arrow-up"  data-art="'+key+'"><div class="icon"></div></button><br>';							
		out+='<button class="btn_arrow-down" data-art="'+key+'"><div class="icon"></div></button>';	
	...
	let inpStatic = doc.getElementsByClassName("product__count_input");
	let btnChangePlus = doc.getElementsByClassName("btn_arrow-up");
	let btnChangeMinus = doc.getElementsByClassName("btn_arrow-down");
	...
	let btnForM2 = doc.getElementsByClassName("mkv");
	let btnForPkg = doc.getElementsByClassName("upak");
	let priceWidthCard = doc.getElementsByClassName("goldPrice");
	let price = doc.getElementsByClassName("retailPrice");
		
		for(let b=0; b<btnForPkg.length; b++){
			 btnForPkg[b].onclick=function(){
				btnForM2[b].removeAttribute('class','product_unit--active');
				btnForPkg[b].setAttribute('class','product_unit--active');
				
			}
		}
		
		for(let a=0; a<btnForM2.length; a++){
			 btnForM2[a].onclick=function(){
				
				btnForPkg[a].removeAttribute('class','product_unit--active');
				btnForM2[a].setAttribute('class','product_unit--active');
			}
		}
	}) 	
